...
export default function Agenda() {
  ...
  const updateGlobalAgenda = (date: number, time: string, data: any) => {
    setMatchSession((prev: any) => {
      ...
      // Se estiver aceitando um cliente, remove todas as outras notificações pendentes para esse mesmo horário
  ...
  }, [selectedDate, globalAgenda, hours24, today, currentHour]);
  ...
  return (
    <div className="flex flex-col bg-white min-h-full font-inter text-blue-950 pb-44 overflow-y-auto no-scrollbar items-center relative">
        ...
        <div className="px-6 py-6 bg-white border-b border-gray-100 sticky top-0 z-50 backdrop-blur-md">
          <div className="flex items-center justify-between mb-8">
            <div className="flex flex-col">
              <div className="flex items-center space-x-2"><span className="w-2 h-2 bg-green-500 rounded-full animate-pulse" /><span className="text-[8px] font-black text-blue-600 uppercase tracking-[0.3em]">Gestão Live: Gustavo 2</span></div>
              <h1 className="text-2xl font-black text-blue-950 uppercase italic tracking-tighter">Agenda Estratégica</h1>
            </div>
            <div className="flex space-x-2">
               ...
               <button onClick={() => isBarberView ? setShowHoursConfig(true) : setIsSelectingServices(false)} className="p-3 bg-gray-900 text-white rounded-2xl shadow-xl">{isBarberView ? <Settings size={20}/> : <Plus size={20}/>}</button>
            </div>
          </div>
                   ...
                   <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={slot.time} className="flex items-start space-x-4 relative z-10">
                      ...
                      <div className="flex-1">
                         <button onClick={() => { 
                            ...
                            } else if (!isBarberView && slot.status === 'empty') {
                            ...
                            <div className="flex items-center space-x-4 text-left">
                               ...
                               <div>
                                  <h4 className={`text-[12px] font-black uppercase tracking-tight ${isMyBooking ? 'text-white' : (slot.status === 'empty' && pendingRequests.length === 0 ? 'text-gray-300' : 'text-blue-950')}`}>
                                     {pendingRequests.length > 1 ? `${pendingRequests.length} SOLICITAÇÕES` : (pendingRequests.length === 1 ? 'NOVA SOLICITAÇÃO' : (isMyBooking ? 'BATALHA C/ GUSTAVO' : slot.client_name))}
                                  </h4>
      ...
      <AnimatePresence>
        ...
        {/* MODAL CONFIG HORÁRIOS (ENGRENAGEM) */}
        {showHoursConfig && (
