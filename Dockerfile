...
# Comando para rodar a aplicação completa com sincronização de banco, seed IBGE e migração de barbeiros
CMD ["sh", "-c", "npx prisma generate && npx prisma db push && node prisma/seed-ibge.js && node prisma/migrate-barbers.js && npm start"]
